...
import {Keypair, Transaction, LAMPORTS_PER_SOL, PublicKey} from '@solana/web3.js';
...
describe('[create_token_vault]', async () => {
    ...
    before(async () => {
         ...
         */
        const signature = await provider.connection.requestAirdrop(keypair.publicKey, LAMPORTS_PER_SOL * 100);
        await workspace.connection.confirmTransaction(signature);

        /**
         * @dev Expect state
         */
        expect(await provider.connection.getBalance(keypair.publicKey)).eq(LAMPORTS_PER_SOL * 100);
    ...
    it('[create_token_vault] should: only administrator can create token vault', async () => {
        ...
        // @ts-ignore
        expect(!!state.allowedMintAccounts.find(item =>
