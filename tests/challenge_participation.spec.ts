...
describe('[challenge_participation]', async () => {
    ...
    before(async () => {
        ...
        vaultTokenAccount = await getAccount(
            workspace.connection,
            (await workspace.programFinder.getTokenVaultAccount(challengeInfo.rewardTokenMintAccount)).address[0]
        );
    ...
    it('[join_challenge] should: anyone can join the challenge with an amount above min deposit', async () => {
       ...
       const player1Ins = await workspace.instructionBuilder.joinChallenge({
        ...
        expect(state.players[0].publicKey.toBase58()).eq(player1.publicKey.toBase58());
        expect(state.players[0].totalDeposit.eq(new BN(LAMPORTS_PER_SOL * 10))).to.be.true;
        expect(state.players[1].publicKey.toBase58()).eq(player2.publicKey.toBase58());
        expect(state.players[1].totalDeposit.eq(new BN(LAMPORTS_PER_SOL * 10))).to.be.true;
        expect(state.players[2].publicKey.toBase58()).eq(player3.publicKey.toBase58());
        expect(state.players[2].totalDeposit.eq(new BN(LAMPORTS_PER_SOL * 10))).to.be.true;
    ...
    it('[claim_reward] should: winners can claim reward successfully', async () => {
        ...
        expect(state.prizePool.eq(new BN(LAMPORTS_PER_SOL * 0))).to.be.true;
        expect(state.donatePool.eq(new BN(LAMPORTS_PER_SOL * 4))).to.be.true;
